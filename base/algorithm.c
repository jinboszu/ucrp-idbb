...
static int *group_src_temp;          // for Rules 10 (SC)
static int *group_src_right;         // for Rule 10 (SC)
static int *group_dst_right;         // for Rule 11 (SD)
...
static bool search(int level, branch_t *branches) {
  ...
  if (level > 0) {
     ...
     * Prepare Rule 10 (SC)
     *
     * group_src_right[s] = max{k | pk == p[s][h[s]] && sk > s &&
     * last_change_type[sk] == MOVE_OUT}
     */
    memset(group_src_temp, 0, sizeof(int) * (max_prio + 1));
    for (int s = n_stacks - 1; s >= 0; s--) {
      group_src_right[s] = group_src_temp[curr_state->p[s][curr_state->h[s]]];
      if (curr_state->last_change_type[s] == MOVE_OUT) {
        int k = curr_state->last_change_time[s];
        int pk = path[k - 1].p;
        if (group_src_temp[pk] < k) {
          group_src_temp[pk] = k;
        }
      }
    }
  ...
  for (int sn = 0; sn < n_stacks; sn++) {
    ...
    int pn = curr_state->p[sn][curr_state->h[sn]]; // priority
    ...
    if (lv > 0) {
       ...
       * Check Rule 1 (TA)
       *
       * if stack sk is unchanged after the block is relocated out
       */
    ...
    if (level > 0) {
       ...
       * Check Rule 3 (TC)
       *
       * if exists s' < s such that h[s'] < n_tiers && last_change_time[s'] < k
       */
    ...
    if (level > 0) {
       ...
       * last_change_type[sk] == MOVE_OUT
       */
      if (curr_state->last_change_time[sn] < group_src_right[sn]) {
    ...
    if (level > 0) {
       ...
       * Prepare Rule 11 (SD)
       *
       * group_dst_right[d] = max{k | pk == pn && dk > d && last_change_type[dk]
       * == MOVE_IN}
       */
      int group_dst_temp = 0;
      for (int d = n_stacks - 1; d >= 0; d--) {
        group_dst_right[d] = group_dst_temp;
        if (curr_state->last_change_type[d] == MOVE_IN) {
          int k = curr_state->last_change_time[d];
          int pk = path[k - 1].p;
          if (pk == pn && group_dst_temp < k) {
            group_dst_temp = k;
          }
        }
      }
    ...
    for (int dn = 0; dn < n_stacks; dn++) {
      ...
      if (level > 0) {
         ...
         */
        if (curr_state->last_change_time[dn] < group_dst_right[dn]) {
       ...
       * Child lower bound
       */
      int child_lb = lb_ts(child_state, array_s1);
      ...
      branches[size].q_src = curr_state->q[sn][curr_state->h[sn]];
...
report_t *solve(instance_t *inst, int _t) {
  ...
  if (init_len_jzw == INT_MAX && init_len_sm2 == INT_MAX) {
    free_state(root_state);
  ...
  group_src_temp = malloc(sizeof(int) * (max_prio + 1));
  group_src_right = malloc(sizeof(int) * n_stacks);
  group_dst_right = malloc(sizeof(int) * n_stacks);
   ...
   * Root lower bound
   */
  int root_lb = lb_ts(root_state, array_s1);
  ...
  debug_info("end");
  print_moves(stdout, best_sol, best_ub);
  fflush(stdout);
  ...
  free(group_src_temp);
  free(group_src_right);
  free(group_dst_right);
  free(path);
  free(hist);
  for (int i = 0; i < max_depth * n_stacks * (n_stacks - 1); i++) {
