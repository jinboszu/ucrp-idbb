...
int lb_ts(state_t *state, int *tau) {
  ...
  memcpy(tau, state->h, sizeof(int) * n_stacks); // initialize tau
  int lowest = INT_MAX; // lowest = min{tau[s] | s=1,...,S}
  ...
  while (lowest > 0) {
    ...
    int q_max = 0;       // q_min = max{q[s][h[s]] | s=1,...,S}
    for (int s = 0; s < n_stacks; s++) {
      if (q_min > q[s][tau[s]]) {
        q_min = q[s][tau[s]];
      }
      if (tau[s] < n_tiers && q_max < q[s][tau[s]]) {
        q_max = q[s][tau[s]];
      }
    }
