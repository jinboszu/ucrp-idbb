...
state_t *malloc_state(int n_stacks, int n_tiers) {
  ...
  state->h = malloc(sizeof(int) * 5 * n_stacks);
  state->last_change_time = state->h + 1 * n_stacks;
  state->last_change_type = state->h + 2 * n_stacks;
  state->last_move_out_time = state->h + 3 * n_stacks;
  state->last_move_in_time = state->h + 4 * n_stacks;
...
void copy_state(state_t *dst_state, state_t *src_state) {
  ...
  memcpy(dst_state->h, src_state->h, sizeof(int) * 5 * dst_state->n_stacks);
...
static void reset_target(state_t *state) {
  if (state->n_blocks == 0) {
    ...
    for (int s = 1; s < state->n_stacks; s++) {
      if (compare(state, s, state->s_min) < 0) {
        state->s_min = s;
      }
    }
  }
...
void retrieve(state_t *state, int l) {
  ...
  state->last_change_type[state->s_min] = RETRIEVE;
  reset_target(state);
}
