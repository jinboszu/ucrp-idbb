...
static bool search(int level, branch_t *branches) {
   ...
   */
  int min_prio =
      curr_state->q[curr_state->s_min][curr_state->h[curr_state->s_min]];
  memset(group_src_temp + min_prio + 1, 0, sizeof(int) * (max_prio - min_prio));
  for (int s = n_stacks - 1; s >= 0; s--) {
    group_src_right[s] =
        curr_state->h[s] == 0
            ? 0
            : group_src_temp[curr_state->p[s][curr_state->h[s]]];
    if (curr_state->last_change_type[s] == MOVE_OUT) {
      int k = curr_state->last_change_time[s];
      int pk = path[k - 1].p;
      if (pk > min_prio && group_src_temp[pk] < k) {
        group_src_temp[pk] = k;
      }
    }
  }
